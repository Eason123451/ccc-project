...
def plot_sentiment_distribution(port):
    # Get data from each endpoint
    busiest_day_data = get_url_data(port, 'search-twitter-busiestday')
    quiest_day_data = get_url_data(port, 'search-twitter-quietestday')
...
def filter_crime_related_tweets(df):
    crime_keywords = [
        'crime', 'criminal', 'police', 'arrest', 'suspect', 'illegal', 'felony', 'misdemeanor',
        ...
        'corruption', 'bribery', 'court', 'trial', 'judge', 'lawyer', 'sentence', 'parole', 'burglary', 'shoplifting',
        'looting', 'extortion', 'embezzlement', 'manslaughter', 'domestic violence', 'stalking', 'identity theft',
        'money laundering', 'break in', 'broke in', 'prostitution', 'human trafficking', 'hate crime', 'gang', 'gang violence',
        'shooting', 'homicide', 'vehicular homicide', 'assassination', 'racketeering', 'conspiracy', 'solicitation'
    ]
...
def sample_crime_quietest_tweets(port):
    # Get data from each endpoint
...
def sample_crime_busiest_tweets(port):
    # Get data from each endpoint
    busiest_day_data = get_url_data(port, 'search-twitter-busiestday')
